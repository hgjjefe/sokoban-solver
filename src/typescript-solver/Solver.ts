...
export class Solver {
    ...
    private distanceMap = new Map<PosInt, Map<PosInt, number>>();
    ...
    private pushActions: Int32Array; // Every three elements are (boxPos, dr,dc), for storing nextPushes in floodRoom
    private pushCount = 0;
    ...
    private getHeuristic(boxPositions: BoxPositions): number {
    ...
    private getWalkPath(startPos: PosInt, targetPos: PosInt, boxPositions: PositionSet): string{
        if (startPos === targetPos) return '';
        const [startR, startC] = getRC(startPos);
        const [targetR, targetC] = getRC(targetPos);
        const queue: Deque<[number, number, string]> = new Deque([[startR, startC, ""]]);
